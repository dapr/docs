...
| [**Workflows**]({{< ref "workflow-overview.md" >}}) | `/v1.0-alpha1/workflow` | The Workflow API enables you to define processes or data flows that span multiple microservices via the Dapr-embedded workflow engine or supported external workflow components. With the Workflow API, you can easily integrate with existing Dapr building blocks while maintaining portability.