...
After [Dapr is installed](https://docs.dapr.io/getting-started), you can start using the service invocation API (via HTTP) by first assigning a unique ID for your app, and then invoke your service with with the `dapr-app-id` header (our reccomended setup). There are multiple ways to get started with the service invocation API but with the `dapr-app-id` header there's no need to change your existing app endpoint URLs — just add the header and you're ready to go. 
