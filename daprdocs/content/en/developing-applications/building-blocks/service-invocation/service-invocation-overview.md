...
- **Direct call to the API** - In addition to proxying, there's also an option to directly call the service invocation API to invoke a GET endpoint. This method is recomended for non-production scenarios like quick demos, general app or scenario testing with Dapr.
