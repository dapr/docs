...
| `workflow.latencyDistributionBuckets` | array | Optional override of the latency distribution buckets used for the workflow and activity execution latency histograms. When unset, these histograms use the same buckets as `latencyDistributionBuckets`. Only the workflow and activity execution latencies are affected; operation and scheduling latencies continue to use the shared buckets. |
| `workflow.latencyDistributionUnits` | duration | The unit the `workflow.latencyDistributionBuckets` values are expressed in, as a Go duration string (for example `1ms` or `1s`). Defaults to `1ms` (milliseconds). Buckets are scaled into milliseconds, which is the unit the histograms are recorded in. |
