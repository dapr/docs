...
The [Scheduler]({{< ref scheduler.md >}}) service is responsible for writing jobs to its embedded database and scheduling them for execution. On installation, a Persistent Volume Claim (PVC) is automatically created using the default Storage Class. The PVC requests storage, which is fulfilled by a Persistent Volume (PV) backed by a physical disk provided by your cloud provider or Kubernetes infrastructure. This durable storage ensures the Scheduler database is resilient to restarts or cluster failure.

You can create the persistent volume on a different storage class by setting the `dapr_scheduler.cluster.storageClassName` parameter during installation.
...
By default, a Persistent Volume with the size of 1GiB will be created on installation, but you can adjust this value to your needs through the `dapr_scheduler.cluster.storageSize` parameter. Disk size should be determined by how many jobs are expected to be persisted at once; however, 64Gb should be more than sufficient for most use cases.
