...
4. **Timeouts** Control the sensitivity of network connectivity between the Placement service and the sidecars using the below timeout values. Default values are set, but you can adjust these based on your network conditions.
   ...
   3. `dapr_placement.disseminateTimeout` sets the time a dissemination round waits for every sidecar to acknowledge before evicting the slow ones. Higher values increase the grace period for slow sidecars, but delay detecting failures. Default is `8s`.
