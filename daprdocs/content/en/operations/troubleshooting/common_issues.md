...
- You may have defined the `NAMESPACE` variable locally or deployed your components into a different namespace in Kubernetes. Check which namespace your app and the components are deployed to.
- You may have not provided a `--components-path` with the Dapr `run` commands or not placed your components into the default components folder for your OS. 
- You may have a syntax issue in component yaml file. Check your component yaml with the component yaml samples in the docs. 