...
| enableTLS          | N  | Verifies the broker's certificate when `"true"`; it doesn't switch the connection to TLS by itself. Use a TLS scheme in `host` (e.g. `pulsar+ssl://broker:6651`) to actually connect over TLS — without one, `host` defaults to plaintext `pulsar://`. The broker must also have TLS configured. Default: `"false"` | `"true"`, `"false"` |
