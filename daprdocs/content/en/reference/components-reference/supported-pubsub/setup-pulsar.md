...
| enableTLS          | N  | Controls certificate validation for TLS connections; it does not by itself switch the connection to use TLS. When `"true"`, the Pulsar client verifies the broker's certificate instead of accepting any certificate. To actually connect over TLS, `host` must also point at the broker's TLS listener using a TLS scheme, for example `pulsar+ssl://your-broker:6651` (Pulsar's default TLS port is `6651`, versus `6650` for plaintext) or an `https://` proxy address. If `host` has no scheme, it defaults to plain, unencrypted `pulsar://`, regardless of this setting. The broker must also have a TLS listener configured; see [Pulsar's TLS encryption docs](https://pulsar.apache.org/docs/en/security-tls-transport/). Default: `"false"` | `"true"`, `"false"` |
