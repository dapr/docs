...
| `maxConcurrentHandlers` | N | Maximum number of concurrent handlers per partition when `enableInOrderMessageDelivery` is `false`. The component pauses receives for the partition while this limit is reached. Must be greater than `0`. Default: `100`. | `"100"`
...
When subscribing to a topic, you can configure the checkpointing frequency in a partition by [setting the metadata in the HTTP or gRPC subscribe request]({{% ref "pubsub_api.md#http-request-2" %}}). The default frequency is `1`. The frequency counts contiguous successfully processed events, or batches for bulk subscriptions.
...
Handler failures retry indefinitely using the configured backoff timing until processing succeeds or the subscription is canceled. In concurrent mode, checkpoints do not advance past an unresolved gap. For example, if events 1, 2, and 4 finish successfully while event 3 remains unresolved, the checkpoint stays at event 2. After event 3 succeeds, the checkpoint can advance through event 4.
