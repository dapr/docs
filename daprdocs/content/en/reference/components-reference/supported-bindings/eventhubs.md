...
| `maxConcurrentHandlers` | N | Input | Maximum number of concurrent handlers per partition when `enableInOrderMessageDelivery` is `false`. This limit also bounds received events outside the in-memory contiguous-success prefix, but does not bound successfully processed events after the last stored checkpoint. The component pauses receives for the partition while the limit is reached. Must be greater than `0`. Default: `100`. | `"100"`
