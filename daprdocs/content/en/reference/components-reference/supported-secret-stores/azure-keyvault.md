...
|--------------------|:--------:|---------|---------|
| vaultName          | Y        | The name of the Azure Key Vault, if you only provide name, it would covert to "[your_keyvault_name].vault.azure.net" in Dapr. If your URL uses other suffix, please provide whole link like "test.vault.azure.cn". we support all suffixes that Azure uses. | `"mykeyvault"`, `mykeyvault.value.azure.cn`
