...
- Normally, this is the path to a file on disk, relative or absolute. For example: `"data.db"` or `"/mnt/data/mydata.db"`.
- The path is interpreted by the SQLite library, so it's possible to pass additional options to the SQLite driver using "URI options". For example, `"path/to/data.db?mode=ro"` opens the database at path `path/to/data.db` in read-only mode. Please refer to the SQLite documentation for all supported [URI options](https://www.sqlite.org/uri.html).
